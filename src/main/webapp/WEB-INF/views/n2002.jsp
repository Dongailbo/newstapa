...
<link rel="shortcut icon" href="/resources/img/n2002/static/favicon.ico">
<title>뉴스타파 20대 국회의원 정치후원금</title>
<link rel="stylesheet" href="/resources/css/n2002/multiselect.css">
<link rel="stylesheet" href="/resources/css/n2002/style.css">
    ...
    <script>
		...
		$(document).ready(function() {
			...
			$(document).on("keyup", ".input-name", function(){
				...
				for(var i = 0 ; i < global_list.data.length ; i++){
					var title = global_list.data[i].name+" "+global_list.data[i].team+" "+global_list.data[i].team_info;
					if(title.indexOf(keyword) > -1){
						subs.push(global_list.data[i])
					}
				}
				sub_list.data = subs;
				initList(sub_list);
			...
			/*				
			$(".multiselect__input").focus(function() {
				$(".multiselect__content-wrapper").show();
			})
			*/
			
			//검색 셀렉트박스 클릭시 option 값 보여진다
			$(".multiselect__tags").click(function() {
				$(".multiselect__content-wrapper").show();
			})
			
			//검색 셀렉트박스에서 마우스 호버 아웃시 셀렉트박스 숨김
			$(document).on("mouseleave", ".multiselect__content-wrapper", function() {
				$(".multiselect__content-wrapper").hide();
			})
			
			
			//셀렉트박스의 옵션값에 호섭시 노란색으로 백그라운드변경
			$(document).on("mouseover", ".multiselect__option", function() {
				if($(this).attr("class").indexOf("active-hover") == -1){
					$(".multiselect__option").removeClass("multiselect__option--highlight")
					$(this).addClass("multiselect__option--highlight")
					
					$(".active-hover").css("background", "#f3f3f3")
					$(".active-hover").css("font-weight", "700")
					$(".active-hover").css("color", "#1f1f1f")
					
					var text = $(".active-hover").find("span:first-child").text();
					$(".active-hover").html("<span>"+text+"</span><span class='afters'>선택됨</span>")

				}
			})
			
			//셀렉트박스에서 선택된값에 호버시 검은색으로 백그라운드변경
			$(document).on("mouseover", ".active-hover", function() {
				var text = $(this).find("span:first-child").text();
				$(this).css("background", "#1f1f1f")
				$(this).css("color", "#fff")
				$(this).html("<span>"+text+"</span><span class='afters'>선택을 취소하려면 클릭하세요</span>")
			})
			
			
			//셀렉트박스에서 옵션 선택시, 선택된거 재선택시에는 else로 들어가며 선택취소가 되고, 선택안되거를 선택시에는 if로 들어간다.
			$(document).on("click", ".multiselect__option, .active-hover", function() {
				if($(this).attr("class").indexOf("active-hover") == -1){
					var seq = $(this).attr("data-seq")
					var text = $(this).find("span:first-child").text();
					
					var options = $(".multiselect__option");
					options.removeClass("active-hover")
					$(".multiselect__option").removeClass("multiselect__option--highlight")
					for(var i = 0 ; i < options.length ; i++){
						if(options.eq(i).attr("data-seq") == seq){
							options.eq(i).css("background", "#f3f3f3")
							options.eq(i).css("font-weight", "700")
							options.eq(i).html("<span>"+text+"</span><span class='afters'>선택됨</span>")
							options.eq(i).addClass("active-hover")
						}else{
							options.eq(i).css("background", "white")
							options.eq(i).css("font-weight", "normal")
							options.eq(i).css("color", "#1f1f1f")
							var sub = options.eq(i).find("span:first-child").text()
							options.eq(i).html("<span>"+sub+"</span>")
						}
					}
					$(".multiselect__input").attr("placeholder", text);
					$(".multiselect__placeholder").text(text);
					settingInfo(seq)
					
				}else{
					
					
					//액티브 된 row를 선택을 취소하려면 클릭 버튼누르면
					$(".multiselect__option").removeClass("active-hover");
					var text = $(this).find("span:first-child").text();
					...
					$(".multiselect__input").attr("placeholder", "국회의원 이름을 입력하세요");
					$(".multiselect__placeholder").text("국회의원 이름을 입력하세요");
				}
				$(".multiselect__content-wrapper").hide();
			})
			...
			var chart = c3.generate({
			    ...
			    data: {
			        ...
			        onclick: function (d, i) { 
			        	...
			        	ajaxCallPost("/api/v1/n2002/info", param, function(res){
		...
		function settingInfo(seq){
			ajaxCallGet("/api/v1/n2002/"+seq, function(res){
				...
				$(".detail-team").text(res.data.team + " | " + res.data.team_info);
				...
				$(".member-detail").slideDown(1000, function(){
					var years = new Array();
					...
					var chart = c3.generate({
		 			    ...
		 			    axis: {
		 			        x:{
		 			       	...
		 			       	y:{
		 			    	    tick: {
		 			    	    values: [100000000, 200000000, 300000000, 400000000, 500000000],
		 			    	      format: function (x) {
		 			    	    	  var val = x+'';
		 			    	    	  val = global.comma(val.substring(0, (val.length-4)))+"만원";
		 			    	    	  return val ;
		 			    	    	  },
		 			    	    	 count: 5
		 			    	    },
		 			...
		 			//하단 300만원 초과 후원자 명단 매핑
		 			//bubbleCanvas2016
		 			chart_300("#bubbleCanvas2016", "2016", res.list)
		 			chart_300("#bubbleCanvas2017", "2017", res.list)
		 			chart_300("#bubbleCanvas2018", "2018", res.list)
		 			chart_300("#bubbleCanvas2019", "2019", res.list)
...
<body>
    ...
    <div class="main">
        <div>
            <img src="/resources/img/n2002/static/mainicon.png" alt="아이콘" class="main-icon">
            <h1 class="title">20대 국회의원 정치후원금</h1>
            <p class="main-paragraph">뉴스타파에서 20대 국회의원의 역대 4년간 후원금을 분석했습니다. 국회의원들은 후원금을 얼마나, 누구에게 받았을까요? 궁금한 국회의원을 검색해 내역을 살펴보세요.</p>
        </div>
    </div>
    <div class="section">
        <div id="app">
...
<!-- 							<li style="display: none;"><span class="multiselect__option"><span>검색 결과가 없습니다.</span></span></li>  -->
<!-- 							<li style="display: none;"><span class="multiselect__option">List is empty.</span></li>  -->
				...
				<div>
					<div class="canvas-mini members">
						<div class="canvas-box canvas-box-border">
							...
							<p class="remark-except">※ 2018년, 2017년, 2016년도는 공직선거가 있는 해로서 평년 모금액(1억 5천만원)의 2배인 3억원까지 모금 가능</p>
						</div> 
				...
				<div class="canvas-mini member-detail" style="display: none;">
					<div class="canvas-box canvas-box-border" style="text-align: center;">
					...
					<div class="canvas-box canvas-box-border">
						...
						<ul class="slct">
							<li class="slct-li slct-select" data-tab="1">
								<input type="radio" value="total"> <label for="pickTotal">전체후원금</label>
							</li> 
							<li class="slct-li" data-tab="2">
								<input type="radio" value="high"> <label for="pickHigh">연간 300만원 이상</label>
							</li>
						</ul>
						<div id="chart"></div> 
						<div id="chart2"></div> 
					...
					<div class="canvas-box canvas-box-border">
						...
						<div>
							...
							<div class="the21st_tab_cont">
								<!--16년 : s -->
								<div>
									<p class="remark"><span style="color: rgb(68, 0, 255);">■ 21대 총선 출마자</span><br><span style="color:#999;font-size: 1.2rem">※ 도넛형태 그래프는 각 후원자들 후원금이 전체 금액에서 차지하는 비율을 나타낸 것입니다.<br>그래프가 표시한 색깔 영역 또는 숫자로 마우스를 이동하고 클릭하세요.<br>'후원금 ~%를 차지하고 있는 후원자의 인적사항과 금액'을 확인할 수 있습니다.</span></p> 
									<p class="error-text" style="display: none;">검색 결과가 없습니다 </p>
									<div id="bubbleCanvas2016"><!--원형--></div>
								</div>
								<!--16년 : e -->
								...
								<div>
									<p class="remark"><span style="color: rgb(68, 0, 255);">■ 21대 총선 출마자</span><br><span style="color:#999;font-size: 1.2rem">※ 도넛형태 그래프는 각 후원자들 후원금이 전체 금액에서 차지하는 비율을 나타낸 것입니다.<br>그래프가 표시한 색깔 영역 또는 숫자로 마우스를 이동하고 클릭하세요.<br>'후원금 ~%를 차지하고 있는 후원자의 인적사항과 금액'을 확인할 수 있습니다.</span></p> 
									<p class="error-text" style="display: none;">검색 결과가 없습니다 </p>
									<div id="bubbleCanvas2017"><!--원형--></div>
								</div>
								<!--17년 : e -->
								...
								<div>
									<p class="remark"><span style="color: rgb(68, 0, 255);">■ 21대 총선 출마자</span><br><span style="color:#999;font-size: 1.2rem">※ 도넛형태 그래프는 각 후원자들 후원금이 전체 금액에서 차지하는 비율을 나타낸 것입니다.<br>그래프가 표시한 색깔 영역 또는 숫자로 마우스를 이동하고 클릭하세요.<br>'후원금 ~%를 차지하고 있는 후원자의 인적사항과 금액'을 확인할 수 있습니다.</span></p> 
									<p class="error-text" style="display: none;">검색 결과가 없습니다 </p>
									<div id="bubbleCanvas2018"><!--원형--></div>
								</div>
								<!--18년 : e -->
								...
								<div class="on">
									<p class="remark"><span style="color: rgb(68, 0, 255);">■ 21대 총선 출마자</span><br><span style="color:#999;font-size: 1.2rem">※ 도넛형태 그래프는 각 후원자들 후원금이 전체 금액에서 차지하는 비율을 나타낸 것입니다.<br>그래프가 표시한 색깔 영역 또는 숫자로 마우스를 이동하고 클릭하세요.<br>'후원금 ~%를 차지하고 있는 후원자의 인적사항과 금액'을 확인할 수 있습니다.</span></p> 
									<p class="error-text" style="display: none;">검색 결과가 없습니다 </p>
									<div id="bubbleCanvas2019"><!--원형--></div>
								</div>
								<!--19년 : e -->
							</div>
						</div>
					...
					<div class="detail-info" style="display: none;">
						...
						<div class="canvas-box canvas-box-border popup detail-info">
							<p class="minititle"><strong class="detail-info-name"></strong> 후원자가 후원한 의원 명단</p> 
							<table class="table">
								<tbody>
									<tr>
					...
					<div class="canvas-box popup close"><p class="close-title">처음으로 돌아가기</p></div>
				</div>
				<!-- 국회의원상세보기 : e -->
			</div>
		</div>
        ...
        <div class="footer__box">
            ...
            <div class="footer__info">
                ...
                <div class="footer__info__margin">
                    ...
                    <p> 전화 : 02-2038-0977 팩스번호 : 02-2038-0978 제보 전화 : 02-2038-8029 | webmaster@newstapa.org</p>
                </div>
                <div class="footer__info__margin">
